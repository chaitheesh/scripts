#!/bin/sh
#version=1
#Cheking whether the host is part of any cf3 ranges or not.
#Removing host mentioned as it is in cf3 ranges.
RED='\033[0;31m'
NC='\033[0m'
...
#Arranging the entered hosts into one by one
...
for i in `cat $1`;do
		...
		for j in `cat $hmdir/tmp-out-cf3-$1 | grep cf3`;do
			...
			#for m in `cat $hmdir/series-ranges `;do
				#if [ $j == $m ] ; then
			grep  "\.\." $j.yaml > /dev/null ; # > /dev/null;
				if [ `echo $?` == 0 ]; then
					eh -e %"$j" | grep $i > /dev/null;
					if [ `echo $?` == 0 ]; then
						#echo "Range $j is part of series ";
						grep -w $i $hmdir/HOST-found-asitisin-cf3range-file-$1 $hmdir/HOST-foundin-subrange-file-$1 | grep -w $j > /dev/null;
						if [ `echo $?` != 0 ]; then
